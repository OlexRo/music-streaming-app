//Кастомный хук для получение данных из singer/all-singers
import {
	useEffect,
	useState
} from 'react';
...
export const useShowSinger = () => {
	const [singers, setSingers] = useState<Singer[]>([]);
	const [error, setError] = useState<string | null>(null);

	const fetchSingers = async () => {
		try {
			const response = await axios.get<Singer[]>('http://localhost:5000/singer/all-singers');
